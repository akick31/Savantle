...
class GameController(
    ...
    fun getDailyPlayer(@RequestParam(required = false) date: String?): ResponseEntity<Any> =
        gameResponseService.getDailyPlayer(date)
    ...
    fun getLiveScreenshot(@PathVariable date: String): ResponseEntity<ByteArray> =
        gameResponseService.getLiveScreenshot(date)

    @GetMapping("/players")
    fun getPlayers(): ResponseEntity<Any> =
        gameResponseService.getPlayers()

    @PostMapping("/guess")
    fun makeGuess(@RequestBody request: GuessRequest): ResponseEntity<Any> =
        gameResponseService.validateGuess(request.playerName, request.date, request.guessNumber)

    @PostMapping("/random-player/new")
    fun newRandomGame(): ResponseEntity<Any> =
        gameResponseService.createRandomGame()

    @GetMapping("/random-player/screenshot/{gameId}")
    fun getRandomGameScreenshot(@PathVariable gameId: String): ResponseEntity<ByteArray> =
        gameResponseService.getRandomGameScreenshot(gameId)

    @PostMapping("/random-player/guess")
    fun randomGuess(@RequestBody request: RandomGuessRequest): ResponseEntity<Any> =
        gameResponseService.validateRandomGuess(request.gameId, request.playerName, request.guessNumber)

    @GetMapping("/available-dates")
    fun getAvailableDates(): ResponseEntity<Any> =
        gameResponseService.getAvailableDates()

    @GetMapping("/random-date")
    fun getRandomDate(): ResponseEntity<Any> =
        gameResponseService.getRandomPastDate()

    @PostMapping("/analytics")
    fun recordAnalytics(@RequestBody body: Map<String, String>): ResponseEntity<Any> =
        analyticsService.recordFromRequest(body)

    @GetMapping("/admin/analytics")
    fun getAnalytics(@RequestParam(defaultValue = "30") days: Int): ResponseEntity<Any> =
        analyticsService.getSummaryResponse(days)

    @PostMapping("/contact")
    fun contact(@RequestBody request: ContactRequest): ResponseEntity<Any> =
        contactService.sendResponse(request)
}
