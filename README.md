...
```
Savantle/
  backend/    Spring Boot API, player curation, screenshot capture
  frontend/   React app
```
...
backend/src/main/kotlin/com/savantle/backend/
  ...
  services/
    DailyPlayerService    Player curation, guess validation, hint logic
    MLBRosterService      Active roster and qualified player IDs from MLB Stats API
    ScreenshotService     Playwright screenshot capture from Baseball Savant
    GameResponseService   HTTP response wrappers
    AnalyticsService      Daily event rollup
...
```
frontend/src/
  pages/        DailyPage, ReplayPage, RandomPage, Shell
  components/
    game/       GamePlay, EndScreen, HintDisplay, PercentileDisplay, PlayerSearch
    layout/     Header, LoadingScreen
    modals/     HowToPlay, StatsModal, SettingsModal, ReplayPickerModal, ContactModal
  hooks/        useGameState, useRandomGameState, useStats, useSettings
  services/     api.ts
  utils/        normalize, share, hints
```
